...
- PDF 恢复描述符生成与父条目唯一管理 note 存储
- 选中条目、PDF 附件、管理 note 的手动补录
- 新增 PDF 附件的 Notifier 自动补录
- 整个用户文库的缺失附件扫描与确认后恢复
- 基于 arXiv 的 URL 规范化、下载、SHA-256 校验与导入回原条目
- 当前父条目的恢复状态面板，显示文件名、状态、来源、最后更新时间、报错信息
- 面向最终用户的中文菜单、确认框和状态面板文案

当前仍未完成或计划在后续版本完善的内容：

- 更多来源站点支持，如 DOI 页面解析、落地页抓取
- 更完整的恢复任务队列和批量操作体验
- 启动时或定时自动扫描
- 群组库支持与更细粒度配置项
...
构建一个 Zotero 插件，实现 **“跨设备 PDF 附件自动恢复”**。
...
1. **恢复任务状态面板**：在当前条目的右侧面板显示恢复任务与状态
2. **恢复状态可视化**：区分 `已下载`、`缺失`、`下载中`、`恢复失败`
3. **恢复操作文档化**：补充 README、多语言说明和发布说明
